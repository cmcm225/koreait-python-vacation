# 상속
#두 개 이상의 클래스가 공통부분이 많을 때
# 이를 상위 클래스를 정의해서 묶어줄수있음
...
# 상속받은 객체 자식은 super(부모) 객체의 데이터 영역이 구분되어있다.
#단, self의 데이터 영역은 super객체의 데이터 영역을 포함한다.
...
class Payment:
    ...
    # pass- 나중에 구현하겠다 에러 방지
    #상속받은 자식클래스에서 오버하이딩하면
    #호출안됨
    def pay(self,price):
        pass

#KakaoPayment -pay오버라이딩 15000원 이상 결제시 5퍼할인
#NaverPayment -pay오버라이딩 20000원 이상 결제시 2000원 할인

class KakaoPayment(Payment):
    #생성자 생략 -생성자를 오버라이딩 할때는
    #super().__init__() 필요
    def pay(self,price):
        #super().pay(price) 오버라이딩 시 필수 아님
        if price >= 15000:
            return price*0.95
