...
for idx, menu in enumerate(menus):
    ...
    for webtoon_item in webtoon_items[:10]:
        try:
            ...
            image_url = webtoon_item_img.get_attribute("src")

            # 제목
            # #content > div:nth-child(1) > ul > li:nth-child(1) > div > a
    ...
    # 요일별로모은 webtoon_dict를 전체이스트에 추가
...
for webtoon_dict in webtoon_list:
    ...
    for webtoon_item in webtoon_items:
        ...
        image_url = webtoon_item["image_url"]
        엑셀한줄 = [title,author,rating,image_url]
        new_ws.append(엑셀한줄)
