...
# 가져온 HTML텍스트를 파이썬 객체화 해줌
# 태그하나를 파이썬 객체 하나로 바꿔 줌
soup = BeautifulSoup(response.text,"html.parser")

# soup(전체 html)에서 div태그이면서, 클래스값이 quote
first_quote_tag= soup.find("div",class_="quote")
# first_quote_tag에서 span태그 이면서, 클래스 값이 text인 태그
quote_text_tag=first_quote_tag.find("span",class_="text")
# quote_text_tag가 가지고있는 문자열 추출
...
quote_author_tag=first_quote_tag.find("small",class_="author")
quote_author=quote_author_tag.text
#print(quote_author)
...
# 총10개 -> find_all()의 len()이 10아면 성공
# 10이 아니라 14,16...-> 조건을 더 엄밀하게
all_quote_tags= soup.find_all("div",class_="quote")
result=len(all_quote_tags)
print(result) # 10이나오면 식별 성공!
...
for quote_tag in all_quote_tags:
    ...
    # 도전! 태그들을 텍스트들을 추출해서 아래 tags에 추가해주세요
    tags = []
    ...
    print(f"{author}({link_url}):{text[:50]}...")
