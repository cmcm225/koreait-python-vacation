...
#호출결과가 값이면 값, 특별한 자료형데이터라면 그 데이터처럼 다룰수있다
result=add(10,20)
print(result)
# f(g())-> g() 호출/실행 -> f() 호출/실행
...
#주민번호를 받아 남자인지 여자인지 리턴
def get_gender(pn):
    #전달받은 주민번호가 타당하지 않으면
    if not (validate_pn(pn)):
        return "유효화지 않은 주민 번호입니다."
    # "991122-3344556"
    gender_digit=pn[7]
