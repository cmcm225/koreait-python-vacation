...
#파일(HHD,SSD) - 반 영구적으로 존재(노트에 적어서 보관)
...
day10_01.py는 "./example.txt" 상대 경로로 접근할 수 있다.
day10_01.py는 "../day09/함수심화.py" 상대경로로 접근 할 수 있다.
...
글자들을 숫자로 바꿔줘야함
이때 필요한 규칙(인코딩)
ascii(영어만),euc-kr(옛),cq949(옛),utf-8(웹표준)
"""

#파일 읽기
#open이 리턴을 해주는건 실제 아닌 데이터가 아니라 특수한 자료형
...
#day01폴더 안에 test폴더 안에 있는 test2.txt 읽고 출력
test2_path="../day01/test/text2.txt"
...
#문자열(txt)말고, python 자료형들을 저장할 파일 형식이 필요함
#[{},{},{},..,{}] 이런 경우가 많다.
#참고) csv, xml, yml
...
#json 저장하기 함수
def save_jason(path, data):
    with open(path, "w", encoding="utf-8") as f:
        json.dump(data, f, ensure_ascii=False, indent=4)

nums=[1,2,3,4,5]
save_jason("./menu.json",nums)
load_json("./menu.json")
print(load_json("./menu.json"))
