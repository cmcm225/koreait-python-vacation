...
#기본값설정이 안되어있는 매개변수는
#기본값 설정이 되어있는 매개변수 뒤에 올 수 있다.
#def hello(nationality="대한민국",name):
#    print(f"[{nationality}]{name}님 안녕하세요")

#파이썬은 매개변수를 들어오는 데이터의 순서로 기억
#기본설정이 되어있는게 나중 순서로 와야 명확하게 할당 가능
...
# 권장)*args 패킹 쓸때는 가장 뒷쪽 매개변수로 설계하자
...
print_user(naem="홍길동", age=20) #kwargs패킹
other_data={
    "name":"김길동",
    "age":20,
    "address":"부산"}
